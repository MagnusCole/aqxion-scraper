...
from circuit_breaker import openai_circuit_breaker, with_circuit_breaker
...
class ContextOptimizer:
    ...
    async def optimize_context(self, content: str, query: str) -> str:
        ...
        # Generate summaries for large chunks in parallel
        optimized_chunks = []
        large_chunks = [chunk for chunk in chunks if chunk.token_count > 10000]
        small_chunks = [chunk for chunk in chunks if chunk.token_count <= 10000]

        print(f"📊 Processing {len(large_chunks)} large chunks and {len(small_chunks)} small chunks")

        if large_chunks:
            # Process large chunks in parallel
            print(f"🚀 Generating summaries in parallel for {len(large_chunks)} chunks...")
            summary_tasks = [self.generate_summary(chunk.content) for chunk in large_chunks]
            summaries = await asyncio.gather(*summary_tasks, return_exceptions=True)

            # Process results
            for chunk, summary_result in zip(large_chunks, summaries):
                if isinstance(summary_result, Exception):
                    print(f"⚠️  Error summarizing chunk {chunk.id}: {summary_result}")
                    optimized_chunks.append(chunk)  # Use original chunk on error
                elif isinstance(summary_result, str) and summary_result:
                    chunk.summary = summary_result
                    # Create summarized version
                    summarized_chunk = ContextChunk(
                        id=f"{chunk.id}_summary",
                        content=summary_result,
                        token_count=self.estimate_tokens(summary_result),
                        metadata={"original_chunk": chunk.id, "is_summary": True}
                    )
                    optimized_chunks.append(summarized_chunk)
                else:
                    optimized_chunks.append(chunk)  # Use original chunk if summary failed
        else:
            print("✅ No large chunks to summarize")

        # Add small chunks as-is
        optimized_chunks.extend(small_chunks)
